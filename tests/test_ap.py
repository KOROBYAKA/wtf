...
def test_ap_execution_mode_0(valid_config):
...
def test_ap_close_client_if_no_client_created(valid_config):
    ap = Ap.build_ap(valid_config)
    ap.close_ssh()

    assert ap.client is None

def test_set_ssh(valid_config, monkeypatch, get_fake_client):
...
def test_close_ssh(valid_config, monkeypatch, get_fake_client):
    monkeypatch.setattr(ap_module, "get_client", get_fake_client)
